...
const credentials = base64.decode(base64Credential).split(":");
...
app.post('/Products', async (req, res) => {
 try{
  ...
  if(!newProducts.product || !newProducts.price || !newProducts.quantity || !newProducts.description || !newProducts.category){
    console.log("Missing required details")
    return res.status(400).send("Missing required details")
  } else{
    console.log("Successfully Created");
...
app.delete('/Products/:id', async (req, res) => {
 try{
  ...
  const deleteId = await usersCollection.deleteOne({ _id: new ObjectId(productId) });
  if(!deleteId){
  console.log("Product Not Found");
  return res.status(404).send("User Not Found");
 } else{
