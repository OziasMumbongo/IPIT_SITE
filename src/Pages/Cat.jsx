...
const Cat = ({ addToCart, cart = [], category }) => {
  const [products, setProducts] = useState([]);
  const [searchQuery, setSearchQuery] = useState('');

  useEffect(() => {
    const cacheKey = `products_${category || 'all'}`; // Unique cache key per category or 'all'
    ...
    if (cachedProducts) {
      ...
      const fetchProducts = async () => {
        try {
          ...
          if (response.ok) {
            setProducts(json);
            localStorage.setItem(cacheKey, JSON.stringify(json)); // Cache the products
          } else {
